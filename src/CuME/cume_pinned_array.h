...
#ifndef CUME_PINNED_ARRAY_H
#define CUME_PINNED_ARRAY_H
...
public:
	 ...
	 */
	PinnedArray(size_t sz) : size(sz) {
		assert(sz > 0);
		cpu_data = new T [ size ];
		cume_new_array_pinned(cpu_data, T, size);
	 ...
	 */
	PinnedArray(size_t sz, T value) : size(sz) {
		assert(sz > 0);
		cpu_data = new T [ size ];
		std::fill(&cpu_data[0], &cpu_data[size], value);
		cume_new_array_pinned(gpu_data, T, size);
		cudaHostGetDevicePointer( &gpu_data, cpu_data, 0 );
		std::fill(&cpu_data[0], &cpu_data[size], value);
		elements_per_row = 10;
	}
